...
if (chat_id) {
    ...
    tg.onEvent('mainButtonClicked', async (event) => {
        ...
        if (name && phone && date && time && gosnum && brand && model && buttonValues && chat_id) {

            try {

                const response = await fetch(`/savedata?name=${encodeURIComponent(name)}&phone=${encodeURIComponent(phone)}&brand=${encodeURIComponent(brand)}&model=${encodeURIComponent(model)}&gosnum=${encodeURIComponent(gosnum)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}&manager=${encodeURIComponent(manager)}&chat_id=${chat_id}&visit=${encodeURIComponent(buttonValues)}`);
    ...
    send.addEventListener('click', async (event) => {
        ...
        if (name && phone && date && time && gosnum && brand && model && buttonValues) {
